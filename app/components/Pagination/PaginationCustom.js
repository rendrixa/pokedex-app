...
const PaginationCustom = (props) => {
  ...
  const pageNumbers = [...new Array(paginationCount)].map((_, index) => {
    ...
    if (isPageNumberFirst || isPageNumberLast || isCurrentPageWithinTwoPageNumbers) {
      ...
      return (
        <div key={pageNumber} aria-current="page" className={`relative cursor-pointer z-10 inline-flex items-center px-2 py-1 text-sm ${pageNumber === activePagination ? "text-white font-black rounded-full bg-[#0075BE]" : "text-black font-semibold"}`}
    ...
    if (!isPageNumberOutOfRange) {
      ...
      return (
        <span className="relative inline-flex items-center px-2 py-1 text-sm font-semibold text-black">...</span>
      )
    }
  ...
  return (paginationCount > 1 &&
    <nav className="flex justify-center" aria-label="Pagination">
      <div onClick={() => activePagination == 1 ? '' : paginationHandler(activePagination - 1)}
        className={`relative inline-flex items-center rounded-l-md px-1 py-1 text-black ${activePagination == 1 ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
      ...
      <div onClick={() => activePagination == paginationCount ? '' : paginationHandler(activePagination + 1)}
        className={`relative inline-flex items-center rounded-r-md px-1 py-1 text-black ${activePagination == paginationCount ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
